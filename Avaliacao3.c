...
typedef struct SVertice {
    ...
    _Bool visitado;
    int distancia; // Nova propriedade para armazenar a distância durante Dijkstra
} TVertice;
...
TVertice *adicionarVertice(int info) {
    ...
    novo->visitado = FALSE;
    novo->distancia = INT_MAX; // Inicializa as distâncias como infinito
...
void dijkstra(TVertice *origem) {
    origem->distancia = 0; // A distância da origem para ela mesma é sempre 0
    ...
    while (1) {
        ...
        // Se todos os vértices foram visitados, sair do loop
        ...
        while (conexaoAtual != NULL) {
            ...
            // Se a nova distância for menor que a distância atual do vizinho, atualize
    ...
    // Exibir as distâncias após o término do algoritmo
